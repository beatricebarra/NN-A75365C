...
Animal = 'Sansa';
expDates = {'20180515'};
Root = fullfile('P:\data_raw\primate\SCS_Monkey\');
...
for iDate = 1 : length(selectedFiles)
    ...
    for iF = 1 : length(selectedFiles{iDate})
        ...
        filenameVICON = [expDate,'_', 'KRG_BrainDecoding-'];
        filenameBKR = [expDate,'_', 'KRG_BrainDecoding'];
...
for iFile = 1 : length(TDTdat)
    ...
    % TDT
    TDTdat{iFile} = processEMGSansa(TDTdat{iFile}, 'filter', 'bipolar16');
    
    
    % BKR
    BKRdat{iFile}.TDT_Trigger.times = VICONdat{iFile}.TDT_Trigger.times - VICONdat{iFile}.BKR_Trigger.times;
    BKRdat{iFile}.TDT_Trigger.samples = floor(BKRdat{iFile}.TDT_Trigger.times * brain_Fs);
...
%%
for iFile =7 %[ 1 7 14 15 18]
    
    
    completetrialPlotSansa(TDTdat{iFile}, VICONdat{iFile})
...
for iFile = 1 : length(TDTdat)
    for iC = 1 : size(TDTdat{iFile}.EMGb, 2)
        ...
        TDTdat{iFile}.envEMGb(:,iC) = highPass(TDTdat{iFile}.EMGb(:,iC), 12207.03, 0.01, 3);
        TDTdat{iFile}.envEMGb(:,iC) = abs(TDTdat{iFile}.envEMGb(:,iC));
        TDTdat{iFile}.envEMGb(:,iC) = lowPass(TDTdat{iFile}.envEMGb(:,iC), 12207.03, 10, 3);
    end
end
...
for iFile = 1 : length(TDTdat)
    for iM = 1 : length(VICONdat{iFile}.events_tokeep.startMov)
        ...
        oldtime = linspace(1, size(sortedFRS1, 2), size(sortedFRS1, 2));
        newtime = linspace(1, size(sortedFRS1, 2), nP);
        for i = 1 : size(sortedFRS1)
            intsortedFRS1(i,:) = interp1(oldtime, sortedFRS1(i,:), newtime);
        end
        for i = 1 : size(sortedFRM1)
            intsortedFRM1(i,:) = interp1(oldtime, sortedFRM1(i,:), newtime);
        end
        allFRS1 = allFRS1+ intsortedFRS1; 
        allFRM1 = allFRM1+ intsortedFRM1; 
...
stickelbow(:,1) =  lowPass(-VICONdat{iFile}.kinematic.x( :, 2), 100, 6, 3);
stickelbow(:,2) =  lowPass(-VICONdat{iFile}.kinematic.z( :,2), 100, 6, 3);
stickwrist(:,1) =  lowPass(-VICONdat{iFile}.kinematic.x(:, 3), 100, 6, 3);
stickwrist(:,2) =  lowPass(-VICONdat{iFile}.kinematic.z(:, 3), 100, 6, 3);
subplot(nplots, 2, [11])
...
grabBKR = eventsIdxsBKR(3)  - eventsIdxsBKR(1); 
clear stickshoulder stickelbow stickwrist
stickshoulder(:,1) = lowPass(VICONdat{iFile}.kinematic.y(:, 1), 100, 6, 3);
stickshoulder(:,2) = lowPass(VICONdat{iFile}.kinematic.z(:, 1), 100, 6, 3);
stickelbow(:,1) =  lowPass(VICONdat{iFile}.kinematic.y( :, 3), 100, 6, 3);
stickelbow(:,2) =  lowPass(VICONdat{iFile}.kinematic.z( :,3), 100, 6, 3);
stickwrist(:,1) =  lowPass(VICONdat{iFile}.kinematic.y(:, 5), 100, 6, 3);
...
subplot(1, 2, [2])
plotsagittalStickDiagram(stickshoulder, stickelbow, stickwrist, ...
    [eventsIdxsVICON(3) eventsIdxsVICON(2)-20 eventsIdxsVICON(2)-20], 0, 2, ...
    repmat([1 0 0], 100, 1),  repmat([0 0 1], 100, 1))
axis equal

...
for  iF = 112%1 : length(featureslabels)
...
%% Violin plot of normalized force
iF = 112; 
...
for  iF = 112%1 : length(featureslabels)
    ...
    stim = stim(not(isoutlier(stim)));
    % 
    normvalue = mean(baseline);
    baseline = baseline./normvalue; 
    lesion = lesion./normvalue; 
    stim = stim./normvalue; 

    percentagedecreaselesion = (1-mean(lesion))*100;
    percentageincreasestim = (mean(stim)-mean(lesion))*100;
    data ={baseline, lesion, stim};
    violin(data)
